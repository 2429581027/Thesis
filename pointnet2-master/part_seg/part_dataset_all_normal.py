...
class PartNormalDataset():
    def __init__(self, root, npoints = 2500, classification = False, split='train', 
                 normalize=True, return_cls_label = False):
...
if __name__ == '__main__':
    d = PartNormalDataset(root = '../data/shapenetcore_partanno_segmentation_benchmark_v0_normal', 
                          split='trainval', npoints=3000)
    
    print(len(d))

    i = 500
    ps, normal, seg = d[i]
    print ('d.datapath[i] = ',d.datapath[i])
    print ('Max, Min = ',np.max(seg), np.min(seg))
    print('shape for ps,seg and normal = ',ps.shape, seg.shape, normal.shape)
    print ('Ps = ',ps)
    print ('Normal = ',normal)
